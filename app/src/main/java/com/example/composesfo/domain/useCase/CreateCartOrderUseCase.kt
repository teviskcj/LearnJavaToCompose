...
class CreateCartOrderUseCase @Inject constructor(
    private val repository: CartRepository
) {
    operator fun invoke(userId: String, orderId: String, foodId: String, cartDto: CartDto): Flow<Resource<Cart>> = flow {
        try {
            emit(Resource.Loading<Cart>())
            repository.createCartOrder(userId,orderId, foodId, cartDto)
